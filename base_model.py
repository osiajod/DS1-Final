...
predictors = ['OFFENSE_CODE_GROUP', 'SHOOTING', 'MONTH', 'DAY_OF_WEEK', 'HOUR',
...
df_cat = enc.fit_transform(df[["SHOOTING", "DAY_OF_WEEK"]]).toarray()

df_cat = pd.DataFrame(df_cat, columns=enc.get_feature_names(['SHOOTING', 'DAY_OF_WEEK']))
...
y_pred = reg.predict(X_normalized_tst)
